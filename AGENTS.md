...
- Rule 的 `.md` 正文末尾加 HTML 来源注释（`<!-- x-source-repo` / `x-source-path` / `x-rule-version` / `x-source-url -->`），取用方去掉 frontmatter 复制后仍能对照上游更新；各 Rule 的功能说明与 Version Notes 写在 `rules/README.md`，通用取用流程见下文「Rule：创建、维护与调用」。
...
- 在该 Rule 条目下写**中文**说明，至少包含：
  ...
  - **使用方法**：链到本文 §3 的通用取用流程；若该 Rule 有特有用法（非通用路径/frontmatter），在此补充。
...
从对外 Rule 复制到目标仓库或用户级配置时：

1. 复制 `rules/<name>.md` 正文（去掉本仓库治理用 frontmatter），**保留**文末 `<!-- x-source-* -->` 注释。
2. 按目标工具补写必要的 frontmatter（如 Cursor 的 `alwaysApply`、`globs`）。
3. **不要**把 `rules/README.md` 里的功能说明、Version Notes 抄进项目本地 Rule 正文。

**常见取用路径**（各 Rule 相同，文件名按 `<name>` 替换）：
...
**对照更新**：读取本地规则文末注释中的 `x-source-url`（或 `x-source-repo` + `x-source-path`），与上游 diff；本地 `x-rule-version` 低于上游时再合并更新（流程见下文 §4）。
...
- **根目录 `README.md`**：在「Rules」清单中按现有风格补一条简要说明（功能 + use example + 指向 `rules/README.md` 功能说明与本文 §3 取用方式）；Rule 删除或重命名时同步更新。
